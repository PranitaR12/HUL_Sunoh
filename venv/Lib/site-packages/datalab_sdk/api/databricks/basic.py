...
from datalab_sdk.api.databricks.exceptions import *
from datalab_sdk.api.databricks.types.metadata import (
 DatabricksResponse
)
...
class BasicDatabricksClient(RawClient):
...
################################################################################
    ...
    def post_request(
        ...
        """Post request for databricks.

        Arguments:
            token: str -- The auth token to use for the API call.
            path: str -- The path of databricks API url
            arg: str -- The argument for databricks API url
            req_data: dict -- Request info that needs to be provided for post request body 

        Returns:
            dict -- Post response of databricks
        ...
        return self.__request_post(
            ...
            data=req_data,
            input_class=dict,
            output_class=DatabricksResponse,
            not_found=None
        )
...
################################################################################
    ...
    def __request_post(
        ...
        # make call
        url = EnvironmentConfig.get_databricks_api_url(self._environment, path, arg)

        # Special case where we don't expect a response
        if output_class is None:
            try:
                response = self.post(url, token, body, parse_response=False)
            except DatabricksNotFound as err:
                if not_found is not None:
                    # Convert databricks specific errors
    ...
    def __request_get(
        ...
        # make call
        url = EnvironmentConfig.get_databricks_api_url(self._environment, path, argument)
