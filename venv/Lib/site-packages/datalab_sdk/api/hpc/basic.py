...
from datalab_sdk.api.hpc.exceptions import *
from datalab_sdk.api.hpc.types.metadata import (
    HpcResponse
)
...
class BasicHpcClient(RawClient):
...
################################################################################
    ...
    def get_request(
        ...
        Returns:
            HpcResponse -- Post response of hpc
...
################################################################################
    ...
    def __request_post(
        ...
        # make call
        url = EnvironmentConfig.get_hpc_api_url(self._environment, path, arg)

        # Special case where we don't expect a response
        if output_class is None:
            try:
                response = self.post(url, token, body, parse_response=False)
            except HpcNotFound as err:
                if not_found is not None:
                    # Convert hpc specific errors
    ...
    def __request_get(
        ...
        # make call
        url = EnvironmentConfig.get_hpc_api_url(self._environment, path, argument)
