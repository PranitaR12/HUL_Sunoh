...
from datalab_sdk.api.plp.exceptions import *
# from datalab_sdk.api.plp.types.basic_metadata import (
...
class BasicPlpClient(RawClient):
...
################################################################################
    ...
    def __request_post(
        ...
        # make call
        url = EnvironmentConfig.get_plp_api_url(self._environment, path, arg)

        # Special case where we don't expect a response
        if output_class is None:
            try:
                response = self.post(url, token, body, parse_response=False)
            except PlpNotFound as err:
                if not_found is not None:
                    # Convert plp specific errors
    ...
    def __request_get(
        ...
        # make call
        url = EnvironmentConfig.get_plp_api_url(self._environment, path, argument)
    ...
    def __request_put(
        ...
        if output_class is None:
            try:
                response = self.put(url, token, body, parse_response=False)
            except PlpNotFound as err:
                if not_found is not None:
                    # Convert plp specific errors
                    raise not_found(err.correlation_id)
