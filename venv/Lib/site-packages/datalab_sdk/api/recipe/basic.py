...
from datalab_sdk.api.recipe.exceptions import *
from datalab_sdk.api.recipe.types.metadata import (
    RecipeRequest
)
...
class BasicRecipeClient(RawClient):
...
################################################################################
    ...
    def post_request(
        self,
        token: str, 
        req_data: RecipeRequest,
        path: str = '',
        arg: str = ''
    ) -> Any:
        """Post request for recipe.

        Arguments:
            token: str -- The auth token to use for the API call.
            path: str -- The path of recipe API url
            arg: str -- The argument for recipe API url
            req_data: dict -- Request info that needs to be provided for post request body 

        Returns:
            Any -- Post response of recipe
...
################################################################################
    ...
    def __request_post(
        ...
        # make call
        url = EnvironmentConfig.get_recipe_api_url(self._environment, path, arg)
    ...
    def __request_get(
        ...
        # make call
        url = EnvironmentConfig.get_recipe_api_url(self._environment, path, argument)
        try:
            # can safely cast to JSONResponseType since parse_json defaults to True
            ...
            if not_found is not None:
                # Convert recipe specific errors
